...
class GameState:
    ...
    def is_round_over(self):
        ...
        # Убедимся, что players_for_comparison не пуст, чтобы избежать деления на ноль или ложных срабатываний
...
def start_next_human_turn(room_id):
    """Начинает ход текущего игрока, запускает таймер и отправляет обновление."""
...
def turn_watcher(room_id, turn_id, time_limit):
    """Фоновая задача, ждет time_limit секунд и проверяет, актуален ли ход."""
...
# --- ИСПРАВЛЕНИЕ: ИСПОЛЬЗУЕМ PARAMS ДЛЯ USER DATA ---
def validate_telegram_data(init_data_str):
    ...
    try:
        ...
        # Разбираем параметры &key=value, сохраняя порядок
        ...
        # Сортируем ключи для data_check_string
        ...
        for key in sorted_keys:
            ...
            if key == 'user':
                user_data_value = value # Сохраняем значение 'user'
        ...
        if calculated_hash == received_hash:
            if user_data_value:
                # Декодируем значение 'user' и парсим JSON
                return json.loads(unquote(user_data_value))
            else:
                print("[AUTH ERROR] Хеш верный, но параметр 'user' не найден.")
                return None
        else:
...
def handle_telegram_login(data):
    ...
    with app.app_context():
        ...
        if user:
            ...
            print(f"[AUTH] Игрок {user.nickname} (TG ID: {telegram_id}, SID: {sid}) вошел через Telegram.")
        else:
...
def handle_set_username(data):
    ...
    with app.app_context():
        ...
        try:
            ...
            db.session.commit()
            add_player_to_lobby(sid)
            print(f"[AUTH] Зарегистрирован новый игрок: {nickname} (TG ID: {telegram_id}, SID: {sid})")
...
def handle_request_skip_pause(data):
    """Обрабатывает запрос на пропуск паузы."""
    ...
    if game.mode == 'solo':
        if game_session.get('pause_id'):
            print(f"[GAME] {room_id}: Пропуск паузы (соло) от {sid}.")
...
def handle_start_game(data):
    ...
    if mode == 'solo':
        ...
        try:
            ...
            if game.num_rounds == 0: # Проверка, что клубы были выбраны
                 print(f"[ERROR] {nickname} ({sid}) попытался начать соло игру без клубов.")
                 leave_room(room_id)
                 add_player_to_lobby(sid)
                 emit('start_game_fail', {'message': 'Не выбраны клубы для игры.'}) # Уведомляем клиента
                 return
            ...
            if room_id in active_games: del active_games[room_id]
            add_player_to_lobby(sid)
            emit('start_game_fail', {'message': 'Ошибка сервера при создании игры.'})


@socketio.on('create_game')
def handle_create_game(data):
    """Создает открытую PvP игру."""
    sid = request.sid
    nickname = data.get('nickname')
    settings = data.get('settings')
    ...
    # Проверка настроек перед созданием
    temp_game = GameState({'nickname': nickname}, all_leagues_data, mode='pvp', settings=settings)
    if temp_game.num_rounds < 3:
         print(f"[ERROR] {nickname} ({sid}) попытался создать игру менее чем с 3 клубами.")
         emit('create_game_fail', {'message': 'Нужно выбрать минимум 3 клуба для игры.'})
         return
    ...
    remove_player_from_lobby(sid)
    print(f"[LOBBY] {nickname} ({sid}) создал комнату {room_id}. Клубов: {temp_game.num_rounds}, Таймбанк: {settings.get('time_bank', 90)}")
    socketio.emit('update_lobby', get_lobby_data_list())

@socketio.on('cancel_game')
def handle_cancel_game():
    """Отменяет созданную игру."""
    sid = request.sid
...
def handle_join_game(data):
    ...
    if not room_id_to_join:
        print(f"[LOBBY] {joiner_nickname} не смог присоединиться к {creator_sid}. Комната не найдена.")
    ...
    try:
        ...
        start_game_loop(room_id_to_join)
         ...
         # Уведомить игроков об ошибке?
         emit('join_game_fail', {'message': 'Ошибка сервера при создании игры.'}, room=p1_info_full['sid'])
         emit('join_game_fail', {'message': 'Ошибка сервера при создании игры.'}, room=p2_info_full['sid'])
...
def handle_submit_guess(data):
    ...
    if game.players[game.current_player_index].get('sid') != sid:
        # Игнорируем ход не от того игрока
        return
