...
# Обязательно установи эту переменную окружения на твоем сервере!
...
class User(db.Model):
    ...
    # НОВОЕ ПОЛЕ: Счётчик сыгранных матчей
...
# --- НОВЫЕ ФУНКЦИИ: Рейтинги и Таблица Лидеров ---
def update_ratings(winner_user_obj, loser_user_obj):
    """Обновляет Glicko-2 рейтинги двух игроков и сохраняет в БД."""
    with app.app_context():
        winner = Player(rating=winner_user_obj.rating, rd=winner_user_obj.rd, vol=winner_user_obj.vol)
        loser = Player(rating=loser_user_obj.rating, rd=loser_user_obj.rd, vol=loser_user_obj.vol)
        ...
        winner_user_obj.rating = winner.rating
        winner_user_obj.rd = winner.rd
        winner_user_obj.vol = winner.vol
        
        loser_user_obj.rating = loser.rating
        loser_user_obj.rd = loser.rd
        loser_user_obj.vol = loser.vol

        db.session.commit()
        print(f"[RATING] Рейтинги обновлены: {winner_user_obj.nickname} vs {loser_user_obj.nickname}")
        ...
        leaderboard = [
            {
                ...
                'games_played': user.games_played  # Добавляем новое поле
            }
            for user in users
        ]
    return leaderboard
...
def start_game_loop(room_id):
    ...
    if not game.start_new_round():
        ...
        print(f"[GAME] Игра в комнате {room_id} окончена. Причина: {game.end_reason}, Счет: {game.scores.get(0, 0)}-{game.scores.get(1, 0)}")
        ...
        if game.mode == 'pvp':
            ...
            if not p1_obj or not p2_obj:
                ...
                # --- ИЗМЕНЕНИЕ: Увеличиваем счётчик сыгранных игр ---
                with app.app_context():
                    p1_obj.games_played += 1
                    p2_obj.games_played += 1
                    db.session.commit()
                    print(f"[STATS] Игрокам {p1_obj.nickname} и {p2_obj.nickname} засчитана игра.")
                # --- Конец изменения ---

                p1_old_rating, p2_old_rating = int(p1_obj.rating), int(p2_obj.rating)
                if game.scores[0] > game.scores[1]: update_ratings(winner_user_obj=p1_obj, loser_user_obj=p2_obj)
                elif game.scores[1] > game.scores[0]: update_ratings(winner_user_obj=p2_obj, loser_user_obj=p1_obj)
                ...
                game_over_data['rating_changes'] = { 'p1': {'old': p1_old_rating, 'new': p1_new_rating}, 'p2': {'old': p2_old_rating, 'new': p2_new_rating} }
...
def show_round_summary_and_schedule_next(room_id):
    ...
    round_result = { 'club_name': game.current_club_name, 'p1_named': p1_named_count, 'p2_named': p2_named_count, 'result_type': game_session.get('last_round_end_reason', 'completed'), 'player_nickname': game_session.get('last_round_end_player_nickname', None) }
    ...
    with app.app_context():
        for room_id, game_info in open_games.items():
            ...
            if creator_user:
                ...
                lobby_list.append({ 'settings': settings_with_clubs, 'creator_nickname': creator_user.nickname, 'creator_rating': int(creator_user.rating), 'creator_sid': game_info['creator']['sid'], 'selected_clubs_names': selected_clubs })
    return lobby_list
    ...
    for room_id, game_session in list(active_games.items()):
        ...
        if disconnected_player_index != -1:
            ...
            if len(game.players) > 1:
                ...
                if game.players[opponent_index]['sid'] != 'BOT': opponent_sid = game.players[opponent_index]['sid']
            break
...
def handle_set_username(data):
    ...
    with app.app_context():
        ...
        db.session.commit()
        add_player_to_lobby(request.sid)
        print(f"[AUTH] Зарегистрирован новый игрок через Telegram: {nickname} (TG ID: {telegram_id})")
...
def handle_start_game(data):
    ...
    if is_player_busy(sid): print(f"[SECURITY] Игрок {nickname} ({sid}) уже занят, попытка начать игру отклонена."); return
    if mode == 'solo':
...
def handle_create_game(data):
    ...
    if is_player_busy(sid): print(f"[SECURITY] Игрок {sid} уже занят, попытка создать игру отклонена."); return
    room_id = str(uuid.uuid4())
    join_room(room_id)
...
def handle_join_game(data):
    ...
    if not room_id_to_join: print(f"[LOBBY] Попытка присоединиться к несуществующей игре. Отклонено."); return
...
def handle_submit_guess(data):
    ...
    if result['result'] in ['correct', 'correct_typo']:
        ...
        if game.time_banks[game.current_player_index] < 0: on_timer_end(room_id); return
