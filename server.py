...
def start_game_loop(room_id):
    game_session = active_games.get(room_id)
    if not game_session: print(f"[ERROR] Попытка запуска цикла для несущ. игры {room_id}"); return
    game = game_session['game']
    ...
    if not game.start_new_round():
        ...
        for spec in spectators:
             if socketio.server.manager.is_connected(spec['sid'], '/'): add_player_to_lobby(spec['sid'])

        if game.mode == 'pvp' and len(game.players) > 1:
            print(f"[RATING_CALC] {room_id}: Начало подсчета рейтинга."); p1_nick, p2_nick = game.players[0]['nickname'], game.players[1]['nickname']
            ...
            with app.app_context():
                try:
                    ...
                    if p1_user and p2_user:
                        p1_old_r, p2_old_r = int(p1_user.rating), int(p2_user.rating); print(f"[RATING_CALC] {room_id}: Старые: {p1_nick}({p1_old_r}), {p2_nick}({p2_old_r})")
                        p1_user.games_played += 1; p2_user.games_played += 1; print(f"[STATS] {room_id}: Засчитана игра.")
                        outcome = 0.5;
                        ...
                        if ratings: p1_new_r, p2_new_r = ratings; print(f"[RATING_CALC] {room_id}: Новые: {p1_nick}({p1_new_r}), {p2_nick}({p2_new_r})")
                        else: print(f"[ERROR][RATING_CALC] {room_id}: update_ratings вернула None."); p1_new_r, p2_new_r = p1_old_r, p2_old_r
                        db.session.commit(); print(f"[RATING_CALC] {room_id}: Изменения сохранены.")
                    ...
                    else: print(f"[ERROR][RATING_CALC] {room_id}: Игроки не найдены."); game_over_data['rating_changes'] = {'0': {'nickname': p1_nick, 'old': p1_old_r, 'new': p1_old_r}, '1': {'nickname': p2_nick, 'old': p2_old_r, 'new': p2_old_r}}
                except Exception as e: db.session.rollback(); print(f"[ERROR][RATING_CALC] {room_id}: Ошибка транзакции: {e}"); game_over_data['rating_changes'] = {'0': {'nickname': p1_nick, 'old': p1_old_r, 'new': p1_old_r}, '1': {'nickname': p2_nick, 'old': p2_old_r, 'new': p2_old_r}}
        else: print(f"[GAME_OVER] {room_id}: Рейтинги не подсчитывались.")

        if room_id in active_games: del active_games[room_id]
        socketio.emit('game_over', game_over_data, room=room_id)
        socketio.close_room(room_id); print(f"[GAME_OVER] {room_id}: Комната закрыта.")
...
def get_open_games_for_lobby():
    ...
    with app.app_context():
        for room_id, game_info in list(open_games.items()):
            ...
            if creator_user: open_list.append({'settings': game_info['settings'], 'creator_nickname': creator_user.nickname, 'creator_rating': int(creator_user.rating), 'creator_sid': game_info['creator']['sid']})
            else: print(f"[LOBBY CLEANUP] User {game_info['creator']['nickname']} not found, removing {room_id}"); del open_games[room_id]
    return open_list
...
@socketio.on('connect')
def handle_connect(): sid = request.sid; print(f"[CONNECTION] Client connected: {sid}"); emit('auth_request')

@socketio.on('disconnect')
def handle_disconnect():
    sid = request.sid; print(f"[CONNECTION] Client disconnected: {sid}"); remove_player_from_lobby(sid)
    room_to_delete = next((rid for rid, g in open_games.items() if g['creator']['sid'] == sid), None)
    if room_to_delete: del open_games[room_to_delete]; print(f"[LOBBY] Creator {sid} disconnected. Room {room_to_delete} removed."); emit_lobby_update()
    ...
    for room_id, game_session in list(active_games.items()):
        game = game_session['game']; idx = next((i for i, p in game.players.items() if p.get('sid') == sid), -1)
        if idx != -1:
            player_game_id = room_id; game_session_player = game_session; disconnected_player_index = idx
            if len(game.players) > 1: opponent_index = 1 - idx;
                if game.players[opponent_index].get('sid') and game.players[opponent_index]['sid'] != 'BOT': opponent_sid = game.players[opponent_index]['sid']
            break
    if player_game_id and game_session_player:
        game = game_session_player['game']; nick = game.players[disconnected_player_index].get('nickname', '?'); print(f"[DISCONNECT] Player {sid} ({nick}) disconnected from {player_game_id}. Game terminated.")
        if game.mode == 'pvp' and opponent_sid:
            print(f"[RATING_CALC_DC] {player_game_id}: Game cancelled. Stats not updated.")
            if socketio.server.manager.is_connected(opponent_sid, '/'): add_player_to_lobby(opponent_sid); emit('opponent_disconnected', {'message': f'Opponent ({nick}) disconnected. Game cancelled, stats not saved.'}, room=opponent_sid); print(f"[GAME] {player_game_id}: Notified {opponent_sid}.")
            else: print(f"[GAME] {player_game_id}: Remaining player {opponent_sid} also disconnected.")
        elif game.mode == 'solo': print(f"[DISCONNECT] {player_game_id}: Player left training.")
        spectators = game_session_player.get('spectators', [])
        for spec in spectators:
            if socketio.server.manager.is_connected(spec['sid'], '/'): emit('opponent_disconnected', {'message': f'Player ({nick}) disconnected. Game ended.'}, room=spec['sid']); add_player_to_lobby(spec['sid']); print(f"[GAME] {player_game_id}: Notified spectator {spec['nickname']}.")
        if player_game_id in active_games: del active_games[player_game_id]
        socketio.close_room(player_game_id); broadcast_lobby_stats(); emit_lobby_update(); return

    spectator_game_id = None
    for room_id, game_session in list(active_games.items()):
        spectators = game_session.get('spectators', [])
        if any(spec['sid'] == sid for spec in spectators):
            spectator_game_id = room_id; game_session['spectators'] = [s for s in spectators if s['sid'] != sid]
            print(f"[SPECTATOR] Spectator {sid} disconnected from {spectator_game_id}."); broadcast_spectator_update(spectator_game_id); broadcast_lobby_stats(); emit_lobby_update(); break

def validate_telegram_data(init_data_str):
    try:
        unquoted = unquote(init_data_str); params = dict(item.split('=', 1) for item in unquoted.split('&') if '=' in item)
        hash_received = params.pop('hash', ''); user_data_value = params.get('user')
        data_check_list = [f"{k}={v}" for k, v in sorted(params.items())]; data_check_string = "\n".join(data_check_list)
        ...
        if calculated_hash == hash_received:
            ...
            else: print("[AUTH ERROR] Hash OK, but no 'user' param."); return None
        else: print(f"[AUTH ERROR] Hash mismatch! Rcvd: {hash_received}, Calc: {calculated_hash}"); return None
...
def handle_telegram_login(data):
    ...
    if not init_data: emit('auth_status', {'success': False, 'message': 'No data.'}); return
    user_info = validate_telegram_data(init_data);
    if not user_info: emit('auth_status', {'success': False, 'message': 'Invalid data.'}); return
    tg_id = user_info.get('id');
    if not tg_id: emit('auth_status', {'success': False, 'message': 'No TG ID.'}); return
    with app.app_context():
        user = User.query.filter_by(telegram_id=tg_id).first()
        if user: add_player_to_lobby(sid); emit('auth_status', {'success': True, 'nickname': user.nickname}); emit_lobby_update(); print(f"[AUTH] {user.nickname} (TG:{tg_id}, SID:{sid}) logged in.")
        else: print(f"[AUTH] New user (TG:{tg_id}, SID:{sid}). Requesting nickname."); emit('request_nickname', {'telegram_id': tg_id})

@socketio.on('set_initial_username')
def handle_set_username(data):
    nick = data.get('nickname', '').strip(); tg_id = data.get('telegram_id'); sid = request.sid
    if not tg_id: emit('auth_status', {'success': False, 'message': 'Error: No TG ID.'}); return
    if not nick or not re.match(r'^[a-zA-Z0-9_-]{3,20}$', nick): emit('auth_status', {'success': False, 'message': 'Nick: 3-20 chars (a-z,0-9,_, -).'}); return
    with app.app_context():
        if User.query.filter_by(nickname=nick).first(): emit('auth_status', {'success': False, 'message': 'Nickname taken.'}); return
        if User.query.filter_by(telegram_id=tg_id).first(): emit('auth_status', {'success': False, 'message': 'TG ID already registered.'}); return
        try:
            new_user = User(telegram_id=tg_id, nickname=nick); db.session.add(new_user); db.session.commit()
            add_player_to_lobby(sid); print(f"[AUTH] Registered: {nick} (TG:{tg_id}, SID:{sid})")
            emit('auth_status', {'success': True, 'nickname': new_user.nickname}); emit_lobby_update()
        except Exception as e: db.session.rollback(); print(f"[ERROR] Create user {nick}: {e}"); emit('auth_status', {'success': False, 'message': 'Registration error.'})
...
def handle_request_skip_pause(data):
    ...
    if not game_session: print(f"[ERROR][SKIP_PAUSE] {sid} skip for non-existent {room_id}"); return
    game = game_session['game']
    if game.mode == 'solo':
        if game_session.get('pause_id'): print(f"[GAME] {room_id}: Skip pause (solo) by {sid}."); game_session['pause_id'] = None; start_game_loop(room_id)
    elif game.mode == 'pvp':
        ...
        if player_index != -1 and game_session.get('pause_id'):
            ...
            if len(game_session['skip_votes']) >= len(game.players): print(f"[GAME] {room_id}: Skip pause (PvP, all votes)."); game_session['pause_id'] = None; start_game_loop(room_id)
...
def handle_get_league_clubs(data):
    league = data.get('league', 'РПЛ'); league_data = all_leagues_data.get(league, {})
    clubs = sorted(list(league_data.keys())); emit('league_clubs_data', {'league': league, 'clubs': clubs})

@socketio.on('start_game')
def handle_start_game(data):
    sid, mode, nick, settings = request.sid, data.get('mode'), data.get('nickname'), data.get('settings')
    if not nick: print(f"[ERROR] Start w/o nickname from {sid}"); return
    if is_player_busy(sid): print(f"[SECURITY] {nick} ({sid}) busy, start rejected."); return
    if mode == 'solo':
        p1_info = {'sid': sid, 'nickname': nick}; room_id = str(uuid.uuid4()); join_room(room_id)
        try:
            game = GameState(p1_info, all_leagues_data, mode='solo', settings=settings)
            if game.num_rounds == 0: print(f"[ERROR] {nick} ({sid}) solo 0 clubs."); leave_room(room_id); add_player_to_lobby(sid); emit('start_game_fail', {'message': 'No clubs.'}); return
            active_games[room_id] = {'game': game, 'turn_id': None, 'pause_id': None, 'skip_votes': set(), 'last_round_end_reason': None, 'spectators': []}
            remove_player_from_lobby(sid); broadcast_lobby_stats(); emit_lobby_update()
            print(f"[GAME] {nick} started training. Room: {room_id}. Clubs: {game.num_rounds}")
            start_game_loop(room_id)
        except Exception as e: print(f"[ERROR] Create solo {nick}: {e}"); leave_room(room_id);
            if room_id in active_games: del active_games[room_id]; add_player_to_lobby(sid)
            emit('start_game_fail', {'message': 'Server error.'}); broadcast_lobby_stats(); emit_lobby_update()

@socketio.on('create_game')
def handle_create_game(data):
    sid, nick, settings = request.sid, data.get('nickname'), data.get('settings')
    if not nick: print(f"[ERROR] Create w/o nickname from {sid}"); return
    ...
    except Exception as e: print(f"[ERROR] Validation {nick}: {e}"); emit('create_game_fail', {'message': 'Settings error.'}); return
    if temp_game.num_rounds < 3: print(f"[ERROR] {nick} ({sid}) pvp < 3 clubs ({temp_game.num_rounds})."); emit('create_game_fail', {'message': 'Min 3 clubs.'}); return
    room_id = str(uuid.uuid4()); join_room(room_id); open_games[room_id] = {'creator': {'sid': sid, 'nickname': nick}, 'settings': settings}
    remove_player_from_lobby(sid); print(f"[LOBBY] {nick} ({sid}) created {room_id}. Clubs: {temp_game.num_rounds}, TB: {settings.get('time_bank', 90)}")
    emit_lobby_update()
...
def handle_cancel_game(data=None):
    ...
    if room_to_delete:
        ...
        print(f"[LOBBY] Creator {sid} cancelled {room_to_delete}."); emit_lobby_update()

@socketio.on('join_game')
def handle_join_game(data):
    joiner_sid, joiner_nick, creator_sid = request.sid, data.get('nickname'), data.get('creator_sid')
    if not joiner_nick or not creator_sid: print(f"[ERROR] Invalid join: {data} from {joiner_sid}"); return
    if is_player_busy(joiner_sid): print(f"[SECURITY] {joiner_nick} ({joiner_sid}) busy, join rejected."); return
    room_id = next((rid for rid, g in open_games.items() if g['creator']['sid'] == creator_sid), None)
    if not room_id: print(f"[LOBBY] {joiner_nick} join to {creator_sid} failed (not found)."); emit('join_game_fail', {'message': 'Game not found.'}); return
    game_info = open_games.pop(room_id); emit_lobby_update()
    creator_info = game_info['creator']
    if creator_info['sid'] == joiner_sid: print(f"[SECURITY] {joiner_nick} join own game {room_id}."); open_games[room_id] = game_info; emit_lobby_update(); return
    p1_info, p2_info = {'sid': creator_info['sid'], 'nickname': creator_info['nickname']}, {'sid': joiner_sid, 'nickname': joiner_nick}
    join_room(room_id, sid=p2_info['sid']); remove_player_from_lobby(p2_info['sid'])
    try:
        ...
        start_game_loop(room_id)
         ...
         print(f"[ERROR] Create PvP {room_id}: {e}"); leave_room(room_id, sid=p1_info['sid']); leave_room(room_id, sid=p2_info['sid'])
         if room_id in active_games: del active_games[room_id]; add_player_to_lobby(p1_info['sid']); add_player_to_lobby(p2_info['sid'])
         emit('join_game_fail', {'message': 'Server error.'}, room=p1_info['sid']); emit('join_game_fail', {'message': 'Server error.'}, room=p2_info['sid'])
         broadcast_lobby_stats(); emit_lobby_update()

@socketio.on('join_as_spectator')
def handle_join_as_spectator(data):
    sid, nick, room_id = request.sid, data.get('nickname'), data.get('roomId')
    if not nick or not room_id: print(f"[ERROR] Invalid spectate: {data} from {sid}"); return
    if is_player_busy(sid): print(f"[SECURITY] {nick} ({sid}) busy, spectate rejected."); emit('spectate_fail', {'message': 'You are busy.'}); return
    game_session = active_games.get(room_id)
    if not game_session: print(f"[SPECTATOR] Game {room_id} not found for {nick}."); emit('spectate_fail', {'message': 'Game not found.'}); return
    my_open_game_id = next((rid for rid, g in open_games.items() if g['creator']['sid'] == sid), None)
    if my_open_game_id: print(f"[SPECTATOR] {nick} ({sid}) spectating, cancelling own game {my_open_game_id}."); handle_cancel_game({'sid': sid})
    join_room(room_id, sid=sid)
    if 'spectators' not in game_session: game_session['spectators'] = []
    game_session['spectators'].append({'sid': sid, 'nickname': nick})
    remove_player_from_lobby(sid); print(f"[SPECTATOR] {nick} ({sid}) joined {room_id}.")
    emit('round_started', get_game_state_for_client(game_session, room_id)); emit('spectate_success', {'roomId': room_id})
    broadcast_spectator_update(room_id); broadcast_lobby_stats(); emit_lobby_update()

@socketio.on('leave_as_spectator')
def handle_leave_as_spectator(data):
    sid, room_id = request.sid, data.get('roomId'); game_session = active_games.get(room_id)
    if not game_session: print(f"[ERROR] Leave non-existent {room_id} spectator {sid}"); return
    initial_spectators = game_session.get('spectators', []); game_session['spectators'] = [s for s in initial_spectators if s['sid'] != sid]
    if len(initial_spectators) > len(game_session['spectators']):
        leave_room(room_id, sid=sid); add_player_to_lobby(sid); print(f"[SPECTATOR] {sid} left {room_id}.")
        broadcast_spectator_update(room_id); broadcast_lobby_stats(); emit_lobby_update()
    else: print(f"[ERROR] Spectator {sid} not found in {room_id} on leave.")

@socketio.on('submit_guess')
def handle_submit_guess(data):
    room_id, guess, sid = data.get('roomId'), data.get('guess'), request.sid; game_session = active_games.get(room_id)
    if not game_session: print(f"[ERROR][GUESS] {sid} guess for non-existent {room_id}"); return
    game = game_session['game']
    if game.players[game.current_player_index].get('sid') != sid: print(f"[SECURITY][GUESS] {sid} not their turn in {room_id}."); return
    result = game.process_guess(guess); nick = game.players[game.current_player_index]['nickname']
...
def handle_surrender(data):
    ...
    if not game_session: print(f"[ERROR][SURRENDER] {sid} surrender non-existent {room_id}"); return
    game = game_session['game']
    if game.players[game.current_player_index].get('sid') != sid: print(f"[SECURITY][SURRENDER] {sid} not their turn in {room_id}."); return
    game_session['turn_id'] = None; game_session['last_round_end_reason'] = 'surrender'; nick = game.players[game.current_player_index]['nickname']
    print(f"[ROUND_END] {room_id}: Player {nick} surrendered."); on_timer_end(room_id)
